...
// the IP address of the iface, send ICMP echo reply; otherwise, translate the
// packet.
void handle_ip_packet(iface_info_t *iface, char *packet, int len)
{
	struct iphdr *ip = packet_to_ip_hdr(packet);							//为什么填充packet+填充struct ？？
	u32 daddr = ntohl(ip->daddr); 								 			 // destinationIP network to host long
	if (ip->protocol == IPPROTO_ICMP && daddr == iface->ip) {         		// iface->ip 是指向物理接口的IP？？？
		struct icmphdr *icmp = (struct icmphdr *)IP_DATA(ip);
		if (icmp->type == ICMP_ECHOREQUEST)
			icmp_send_packet(packet, len, ICMP_ECHOREPLY, 0);   			//ICMP控制消息协议 echo request 例如:ping
		free(packet);
	}
	else {
