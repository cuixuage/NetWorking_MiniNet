...
struct iphdr {
    ...
    u8 ttl;								// ttl of ip packet               //这是time to live 用于防止数据包在互联网或其他网络上不断循环。在转发IP数据包时，要求路由器至少减少一个TTL
    u8 protocol;						// upper layer protocol, e.g. icmp, tcp, udp     //****传输层将使用何种协议
...
// initialize ip header filling the given arguments into ip header fields
void ip_init_hdr(struct iphdr *ip, u32 saddr, u32 daddr, u16 len, u8 proto);

// send an ip packet, it will lookup the forwarding table to determine 
// by which interface to emit it, and initialize the ethernet header by 
// lookup arp cache
