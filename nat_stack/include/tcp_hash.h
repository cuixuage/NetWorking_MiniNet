#ifndef __TCP_HASH_H__
#define __TCP_HASH_H__

#include "hash.h"
#include "hash_list.h"
#include "tcp_sock.h"

#define TCP_HASH_SIZE HASH_8BITS
#define TCP_HASH_MASK (TCP_HASH_SIZE - 1)

struct tcp_hash_table {
	struct hash_list_head established_table[TCP_HASH_SIZE];
	struct hash_list_head listen_table[TCP_HASH_SIZE];
	struct hash_list_head bind_table[TCP_HASH_SIZE];
};

static inline int tcp_hash_function(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	int result = hash8((char *)&saddr, 4) ^ hash8((char *)&daddr, 4) ^ \
				 hash8((char *)&sport, 2) ^ hash8((char *)&dport, 2);

	return result & TCP_HASH_MASK;
}

static inline int tcp_sock_in_table(struct tcp_sock *tsk, struct hash_list_head *list)
{
	struct tcp_sock *tmp;
	hash_list_for_each_entry(tmp, list, hash_list) {
		if (tsk->sk_saddr == tmp->sk_saddr && \
				tsk->sk_daddr == tmp->sk_daddr && \
				tsk->sk_sport == tmp->sk_sport && \
				tsk->sk_dport == tmp->sk_dport)
			return 1;
	}

	return 0;
}

#endif
